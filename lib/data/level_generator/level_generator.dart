...
class LevelGenerator {
  ...
  static LevelModel generateLevel(int levelNumber) {
    ...
    final int maxAttempts = isLargeGrid ? (type == LevelType.normal ? 80 : 120) : 80;
  ...
  static LevelModel? _attempt({
    ...
    // ═══════════════════════════════════════════════════════════════════════
    {
      ...
      int veryLongMax = max(veryLongMin + 1, (veryLongMin + 4).clamp(veryLongMin + 1, mask.length));
      ...
      final int maxAllowedBlocks = (type == LevelType.tutorial || gridSize > 20) ? 0 : 1;
      ...
      while (failures < maxFailures &&
        ...
        final int curVeryLongMax = max(curVeryLongMin + 1, veryLongMax - relaxation);
        ...
        for (final cell in maskCells) {
          ...
          if (freeNeighbours >= 2) { anyCanFit3 = true; break; }
        }
        ...
        if (bestPath == null && type != LevelType.tutorial) {
          ...
          if (bestPath == null && (wantTier == _LenTier.veryLong || wantTier == _LenTier.long)) {
            for (final cand in candidates.take(15)) {
    ...
    if (type != LevelType.tutorial) {
        ...
        while (madeProgress) {
          ...
          for (final cell in emptyCells) {
            ...
            for (final nb in nbOffsets) {
              ...
              if (nb[0] == 1) {         // neighbour is below
                dir1 = ArrowDirection.up;    dir2 = ArrowDirection.down;
              } else if (nb[0] == -1) { // neighbour is above
                dir1 = ArrowDirection.down;  dir2 = ArrowDirection.up;
              } else if (nb[1] == 1) {  // neighbour is right
                dir1 = ArrowDirection.left;  dir2 = ArrowDirection.right;
              } else {                  // neighbour is left
                dir1 = ArrowDirection.right; dir2 = ArrowDirection.left;
              }
              ...
              if (_canExitClean(r, c, dir1, occupiedPacked, gridSize)) {
                headRow = r; headCol = c; tailRow = tr; tailCol = tc; chosenDir = dir1;
              } else if (_canExitClean(tr, tc, dir2, occupiedPacked, gridSize)) {
                headRow = tr; headCol = tc; tailRow = r; tailCol = c; chosenDir = dir2;
              } else {
              ...
              occupied.add('$r,$c'); occupied.add('$tr,$tc');
              occupiedPacked.add(r * 1000 + c); occupiedPacked.add(tr * 1000 + tc);
              madeProgress = true;
          ...
          for (final cell in emptyCells) {
            ...
            for (final nb in nbOffsets) {
              ...
              for (final t in tries) {
                ...
                // Temporarily place
                final tempArrows = List<ArrowModel>.from(arrows)..add(newArrow);
                ...
                final order = (gridSize > 20) ? _greedySolve(tempLevel) : null;
                final isSolvable = (gridSize > 20)
                    ? (order != null)
                    : (LevelSolver.solve(tempLevel, 2000) != null);

                if (isSolvable) {
                  if (gridSize > 20) {
                    File('attempt_log.txt').writeAsStringSync('  Phase 2c solvable: placed ${newArrow.id} at [${newArrow.row}, ${newArrow.col}] (dir: ${newArrow.direction.name}) -> clearing order length: ${order!.length}\n', mode: FileMode.append, flush: true);
                  }
                  arrows.add(newArrow);
                  counter++;
                  occupied.add('$hr,$hc'); occupied.add('$tailR,$tailC');
                  occupiedPacked.add(hr * 1000 + hc); occupiedPacked.add(tailR * 1000 + tailC);
                  madeProgress = true;
                  placedSolvably = true;
                  break;
                }
      ...
      final remainingEmpty = maskCells
          .where((cell) => !occupiedPacked.contains(cell[0] * 1000 + cell[1]))
          .toList();
      final remainingEmptyPacked = remainingEmpty.map((c) => c[0] * 1000 + c[1]).toSet();
      ...
      if (hasAdjacentEmpty) {
        File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: adjacent empty cells left (2 dots together)\n', mode: FileMode.append, flush: true);
        return null;
      }
    }
    ...
    if (fillEntireGrid && emptyCount > maxOrphans) {
      File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: too many orphans $emptyCount / $maxOrphans\n', mode: FileMode.append, flush: true);
      return null;
    }
              ...
              if (shouldColor) {
                ...
                if (!tooClose) {
                  ...
                  for (final candDir in turns) {
                    ...
                    final isSolvable = _greedySolveWithMap(gridSize, arrows, orphanMap) != null;
    ...
    if (levelNumber == 2 || (type != LevelType.tutorial && levelNumber >= 4)) {
      _mechanicMix(arrows, levelNumber, type, rng, gridSize, orphanDots, gridSize);
    }
    ...
    if (gridSize > 20) {
      ...
      if (greedyOrder == null) {
        File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: greedy FAILED (deadlock)\n', mode: FileMode.append, flush: true);
        return null;
      }
      File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: SUCCEEDED (greedy)\n', mode: FileMode.append, flush: true);
      return level.copyWith(solutionOrder: greedyOrder);
    }

    final quickSolution = LevelSolver.solve(level, 5000);
    if (quickSolution == null) {
      File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: solver failed\n', mode: FileMode.append, flush: true);
      return null;
    }
    File('attempt_log.txt').writeAsStringSync('Level $levelNumber Attempt $attempt: SUCCEEDED\n', mode: FileMode.append, flush: true);
  ...
  static bool _canExitWithDeflections(
    ...
    while (nr >= 0 && nr < gridSize && nc >= 0 && nc < gridSize) {
      final keyPacked = nr * 1000 + nc;
      if (visitedCells.contains(keyPacked)) return false; // Loop within this path trace
  ...
  static void _mechanicMix(List<ArrowModel> arrows, int level,
    ...
    for (int i = 0; i < stdIndices.length && actualPairs < pairs; i++) {
      ...
      for (int j = i + 1; j < stdIndices.length; j++) {
        ...
        for (final pt in arrowKey.path) allOtherThanKey.remove('${pt[0]},${pt[1]}');

        final allOtherThanLock = Set<String>.from(allCells);
        for (final pt in arrowLock.path) allOtherThanLock.remove('${pt[0]},${pt[1]}');

        final keyClear = _simulateExitClear(arrowKey, gridSize, allOtherThanKey, orphanMap);
        final lockClear = _simulateExitClear(arrowLock, gridSize, allOtherThanLock, orphanMap);
        ...
        if (keyClear && lockClear) {
          ...
          final tempLevel = LevelModel(
            ...
            patternName: 'temp',
            difficulty: Difficulty.easy,
            mask: arrows.expand((a) => a.path.map((p) => '${p[0]},${p[1]}')).toSet(),
            orphanDots: orphanDots,
          );
  ...
  static int _countBlockedEmptyCells({
    ...
    bool isOccupied(int packed) {
      return currentOccupiedPacked.contains(packed) || newPathPacked.contains(packed);
    }
  ...
  static void _absorbOrphans(List<ArrowModel> arrows, Set<String> occupied, Set<int> occupiedPacked, Set<String> mask) {
  ...
  static bool _canExitClean(
      int headRow, int headCol, ArrowDirection dir,
    ...
    while (remaining > 0) {
      ...
      for (int i = arrows.length - 1; i >= 0; i--) {
        ...
        while (nr >= 0 && nr < gridSize && nc >= 0 && nc < gridSize) {
          ...
          if (visited.contains(idx)) { canExit = false; break; }
          visited.add(idx);
          ...
          if (orphanActive[idx]) {
            ...
            final val = board[idx];
            if (val != 0 && val != i + 1) { canExit = false; break; }
          }
        ...
        if (canExit) {
          ...
          for (final idx in consumed) orphanActive[idx] = false;
          order.add(arrows[i].id);
          remaining--;
          clearedAny = true;
          break;
        }
      ...
      if (!clearedAny) return null;
    }

    return order;
  }

  /// Greedily simulates the game: on each pass, clears every arrow whose exit
  /// path is currently unblocked (accounting for active orphan dots and
  /// already-cleared arrows). Returns the clearing order (arrow IDs) on
  /// success, or null if any arrows remain permanently blocked (deadlock).
  ///
  /// Complexity: O(N² × gridSize) — fast even for 200-arrow 30×30 levels.
  static List<String>? _greedySolve(LevelModel level) {
    ...
    // Flat board: index = row*gridSize+col, value = 1-based arrow index (0=empty).
    final board = Uint16List(gridSize * gridSize);
    for (int i = 0; i < arrows.length; i++) {
      for (final pt in arrows[i].path) {
        board[pt[0] * gridSize + pt[1]] = i + 1;
    ...
    // Orphan dot state.
    // type index: 0=up 1=down 2=left 3=right 4=neutral
    final orphanTypes  = Uint8List(gridSize * gridSize);
    final orphanActive = List<bool>.filled(gridSize * gridSize, false);
    for (final od in orphanDots) {
    ...
    while (remaining > 0) {
      ...
      // Try arrows in reverse-placement order (last placed exits easiest first).
      for (int i = arrows.length - 1; i >= 0; i--) {
        if (!active[i]) continue;
        ...
        while (nr >= 0 && nr < gridSize && nc >= 0 && nc < gridSize) {
          final idx = nr * gridSize + nc;
          if (visited.contains(idx)) { canExit = false; break; } // loop
          visited.add(idx);
          ...
          if (orphanActive[idx]) {
            ...
            // t == 4 (neutral): no direction change
          } else {
            final val = board[idx];
            if (val != 0 && val != i + 1) { canExit = false; break; } // blocked
          }
        ...
        if (canExit) {
          // Clear this arrow and consume orphan dots it passed through.
          active[i] = false;
          for (final pt in arrows[i].path) board[pt[0] * gridSize + pt[1]] = 0;
          for (final idx in consumed) orphanActive[idx] = false;
          order.add(arrows[i].id);
          remaining--;
          clearedAny = true;
          break; // Restart pass — newly cleared cells may unblock other arrows.
        }
      }

      if (!clearedAny) {
        final deadlocked = <String>[];
        for (int i = 0; i < arrows.length; i++) {
          if (active[i]) {
            deadlocked.add('${arrows[i].id}(dir:${arrows[i].direction.name},head:${arrows[i].path[0]},tail:${arrows[i].path.last})');
          }
        }
        File('attempt_log.txt').writeAsStringSync('  Deadlocked arrows: $deadlocked\n', mode: FileMode.append, flush: true);
        return null; // Deadlock: no arrow can exit.
      }
    }
